...
class omiBot(sc2.BotAI):
    ...
    async def on_step(self, iteration):
        ...
        if iteration == 0:
            print(self.units.ready.noqueue.owned, flush=True)
    ...
    async def build_pylons(self):
        # TODO: take account of already pending pylons
        if self.supply_left < 10 and not self.already_pending(PYLON):
            nexuses = self.units(NEXUS).ready
            # TODO: select closest nexus
        ...
        for nexus in self.units(NEXUS).ready:
            ...
            for vaspene in vaspenes:
                if not self.can_afford(ASSIMILATOR):
                    break
                # TODO: worker not carrying resources
